...
* **지원하지 않는 리소스:** 뷰(Views), 사용자 정의 함수(UDF/Routines), 외부 테이블(External Tables), 크로스 리전 CDC 테이블은 복사되지 않습니다.
...
* **데이터 전송 비용 (Network Egress):** BigQuery Data Replication Egress 요율이 적용됩니다.
  ...
  * **[주의]** 복사 작업을 도중에 취소(Cancel)하더라도, 취소 시점까지 이미 네트워크로 전송된 데이터에 대해서는 요금이 부과됩니다.
